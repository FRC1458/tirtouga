...
public class Infrared extends edu.wpi.first.wpilibj.SensorBase {
	...
	private final double[] curveArrayB = { 2.718, 2.392, };// For B which is a
															// 45

	private double[] averageArray = new double[100];

	/**
	 * Constructor for infrared
	 * 
	 * @param inputPort
	 *            Analog port the infrared sensor is in
	 * @param curveType
	 *            Which curve to use to convert voltage to distance, 0=raw
	 *            voltage, 1=slight calculated curve, 2=1/voltage, 3=Kyle
	 *            supercalibrate for 5Y, 4= semicalculated for B
	 */
...
/**
 * Updates the infrared sensor.
 */
	public void update() {
		distance = curve(analogInput.getVoltage(), curveType);
		/*
		 * average = (count * average + distance) / (count + 1); count++;
 ...
 */
	...
	public double getRawVoltage() {
		return curve(analogInput.getVoltage(), 0);
	}
